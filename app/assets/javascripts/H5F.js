    ...
    checkField = function(e) {
        ...
        if(nodes.test(el.nodeName) && !(ignoredTypes.test(el.type) || ignoredTypes.test(el.nodeName))) {
            ...
            if(el.validity.valid && (el.value !== "" || specialTypes.test(el.type)) || (el.value !== el.getAttribute("placeholder") && el.validity.valid)) {
                if (el.id == 'user_password_confirmation' && curEvt == 'focus' && el.value != el.parentNode.previousElementSibling.childNodes[1].value) {

                } else {
                    removeClass(el,[args.invalidClass,args.requiredClass], type);
                    addClass(el,args.validClass, type);
                }
            } else if(!events.test(curEvt)) {
                if(el.validity.valueMissing) {
                    removeClass(el,[args.invalidClass,args.validClass], type);
                    addClass(el,args.requiredClass, type);
                } else if(!el.validity.valid) {
                    removeClass(el,[args.validClass,args.requiredClass], type);
                    addClass(el,args.invalidClass, type);
                }
            } else if(el.validity.valueMissing) {
                removeClass(el,[args.requiredClass,args.invalidClass,args.validClass], type);
            }
